...
function changeOrder(mid){
	...
	obj = '<tr id="dishItem_'+mid+'"><td width="50%" height="30px">'+$("#dishName_"+mid).html()+'</td><td  width="15%" id="weiSinglePrice_'+mid+'">'+$("#weiSinglePrice_"+mid).html()+'</td><td  width="15%"><span id="dishSingleCopies_'+mid+'">1</span> &nbsp; <input type="button" onclick="del('+mid+')" class="buttond" value="删1"></td><td  width="30%" id="dishSingleTotalPrice_'+mid+'">'+dishSingleTotal+'</td></tr>';
...
function del(mid) {
	...
	if (nums > 1)  //超过或者两份
	{
		...
		$("#dishSingleCopies_"+mid).html(parseFloat($("#dishSingleCopies_"+mid).html())-1);
		$("#dishSingleTotalPrice_"+mid).html(parseFloat($("#dishSingleTotalPrice_"+mid).html()) - dishSingleTotal);
		if($("#marketSingleTotalPrice_"+mid)){
			$("#marketSingleTotalPrice_"+mid).html(parseFloat($("#marketSingleTotalPrice_"+mid).html()) - marketSingleTotal);	
		}
		$("#dishBtnNum_"+mid).html(parseFloat($("#dishBtnNum_"+mid).html())-1);
		$("#countNums").html(tag);
	...
	$('#upask').submit(function(){
	   ...
	   if ($('#phone').val().length <8)
	   {
			...
			/*for(var p in jsArr)
			{
				 if(jsArr[p] > 0)
				 {
					$('#inputask').val($('#inputask').val() + p + '_' + jsArr[p] + '|');
				 }
			}*/
			
			/*
			$.each(jsArr, function(key, val) {
